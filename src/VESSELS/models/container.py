...
def Lcontainer(x, ui, U0=7.0):
    ...
    nudot = np.linalg.inv(T @ M @ Tinv) @ ((U**2 / L) * T @ b * delta - (U / L) * T @ N @ Tinv @ nu - (U / L)**2 * T @ G @ Tinv @ eta)
    
    # Dimensional state derivatives
    xdot = np.array([0, nudot[0], nudot[1], np.cos(psi)*U - np.sin(psi)*np.cos(phi)*v, np.sin(psi)*U + np.cos(psi)*np.cos(phi)*v, np.cos(phi)*r, nudot[2], p, delta_dot]).T
