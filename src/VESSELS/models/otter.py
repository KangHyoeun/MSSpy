...
from LIBRARY.kinematics.kinematics import Smtrx, Hmtrx, Rzyx, eulerang
from LIBRARY.modeling.modeling import added_mass_surge, cross_flow_drag, m2c
...
def otter(*args, **kwargs):
    ...
    nu_c_dot = np.concatenate((-Smtrx(nu2) @ nu_c[0:3], np.zeros(3)))
    ...
    MRB_CG = np.block([
        [(m + mp) * I3, np.zeros((3, 3))],
        [np.zeros((3, 3)), Ig]
    ])
    CRB_CG = np.block([
        [(m + mp) * Smtrx(nu2),        np.zeros((3, 3))],
        [np.zeros((3, 3)),    -Smtrx(Ig @ nu2)]
    ])
    ...
    LCF = -0.2
    H = Hmtrx(np.array([LCF, 0, 0]))               # transform G_CF from the CF to the CO 
    G = H.T @ G_CF @ H
    ...
    for i in range(0,2):
        ...
        if n[i] > 0:                          
            Thrust[i] = k_pos * n[i] * np.abs(n[i])    # positive thrust (N) 
        else:
            Thrust[i] = k_neg * n[i] * np.abs(n[i])    # negative thrust (N) 
