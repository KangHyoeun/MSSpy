...
if __name__ == "__main__":
    ...
    waypoints = np.array([[0, 0], [2000, 0], [5000, 5000], [3000, 8000],
                          [6000, 12000], [10000, 12000]])
    ...
    for i in range(nTimeSteps):
        ...
        if ControlFlag == 1:
            ...
            # PID heading autopilot
            e = ssa(psi - psi_d)
            delta_PID = (T / K) * a_d + (1 / K) * r_d - Kp * (
                e + Td * (r - r_d) + (1 / Ti) * e_int)  # Feedforward, PID

            # Reference model propagation
            psi_d, r_d, a_d = ref_model(psi_d, r_d, a_d, psi_ref, r_max,
                                        zeta_d, wn_d, h, 1)
            ...
            chi_d, omega_chi_d = LOS_observer(chi_d, omega_chi_d, chi_ref, h,
                                              K_f)
            ...
            e = ssa(chi_hat - chi_d)
            delta_PID = (1 / K) * omega_chi_d - Kp * (
                e + Td * (omega_chi_hat - omega_chi_d) + (1 / Ti) * e_int)

        delta_c = sat(delta_c, np.deg2rad(40))  # Maximum rudder angle

        # Store data for presentation
        simdata[i] = np.array(x.tolist() + [
            psi_d, r_d, chi_d, omega_chi_d, delta_c, U_hat, chi_hat,
            omega_chi_hat
        ])
        ...
        if i == 0:
            x_hat = EKF_5states(xpos, ypos, h, Z, 'NED',
                                100 * np.diag([0.1, 0.1]),
                                1000 * np.diag([1, 1]), 0.00001, 0.00001,
                                x_prd_init)
        else:
            x_hat = EKF_5states(xpos, ypos, h, Z, 'NED',
                                100 * np.diag([0.1, 0.1]),
                                1000 * np.diag([1, 1]), 0.00001, 0.00001, None)
    """PLOTS"""
    ...
    vehicleData = [('Length', '160.93m'), ('Mass', '17,045 tonnes'),
                   ('Max speed', '7.71m/s'), ('Max rudder angle', '40 deg')]
